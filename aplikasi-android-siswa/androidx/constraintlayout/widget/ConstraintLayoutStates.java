...
public class ConstraintLayoutStates {
    ...
    private void parseConstraintSet(Context context, XmlPullParser parser) {
        ...
        while (i < count) {
            ...
            if (name == null || s == null || !UploadTaskParameters.Companion.CodingKeys.id.equals(name)) {
                i++;
            } else {
                int id = -1;
                if (s.contains("/")) {
                    id = context.getResources().getIdentifier(s.substring(s.indexOf(47) + 1), UploadTaskParameters.Companion.CodingKeys.id, context.getPackageName());
                }
                if (id == -1) {
                    if (s.length() > 1) {
